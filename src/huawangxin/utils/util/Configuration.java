...
public class Configuration {
	...
	/**
	 * 读取配置文件 生成Configuration对象
	 */
	public static Configuration configure() {
		try {
			InputStream in = Configuration.class
					.getResourceAsStream("mysql.xml");
	 ...
	 * 读取配置文件 生成Configuration对象
	 */
	public static Configuration configure2() {
		try {
			InputStream in = Configuration.class
					.getResourceAsStream("mysqlt.xml");
			Configuration cfg = load(in);
			return cfg;
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static Configuration load2(InputStream in) throws DocumentException{
		...
		Configuration cfg = new Configuration(username, password, hostName, smtpPort, charset, subject, sslOnConnect, from, addcontent);
		return cfg;
	}
